...
import { submitPostgraduateApplication } from "../../API/postgraduate";
...
export const SectionSubmit: React.FC = () => {
  ...
  const { prevPageHandler, nextPageHandler } = useProgress({
    page: 8,
    section: "submit",
    nextPageURL: "/postgraduate/section-submit",
    prevPageURL: "/postgraduate/section6",
  });

  const page = searchParams.get("page");

  const { isLoading, mutate } = useMutation({
    mutationFn: submitPostgraduateApplication,
    onSuccess: (response) => {
  ...
  const applicationSubmitHandler = () => {
    const stringifiedData = localStorage.getItem("postgraduate");
    if (!stringifiedData) {
      dispatch(
        showCardNotification({
          type: "error",
